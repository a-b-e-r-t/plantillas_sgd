...
import { getDocumentsAll, getDocumentsProf, removeDocument, saveDocument } from "@/actions/documento";
...
function DocumentoContainer() {
  ...
  const getDoc = async () => {
    const response = await getDocumentsProf();
    setDocs(response);
  };

  const getAllDocs = async () => {
    const response = await getDocumentsAll();
    setAllDocs(response);
  };
  ...
  const handleSave = async (cdoc_tipdoc: string) => {
    ...
    try {
      await saveDocument(cdoc_tipdoc);
      toast.dismiss()
  ...
  const handleRemove = async (cdoc_tipdoc: string) => {
    ...
    try {
      await removeDocument(cdoc_tipdoc);
      toast.dismiss()
  ...
  return (
    ...
    <div className="space-y-4">
      <div className="bg-card border rounded-lg p-4 space-y-4">
        <h1 className="font-semibold text-lg">Agregar nuevo documento</h1>
